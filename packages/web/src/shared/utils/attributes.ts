import type { ClassName, CSSProperties, ElementsAttributes } from '@suisei/htmltypes';
...
[
	'tabIndex',
	'crossOrigin'
].forEach(name => standardAttributeNames.set(name, name));

const camelize = (name: string): string => name.replace(/[\-\:]([a-z])/g, token => token[1].toUpperCase());
...
[
	...
	'spellCheck',
	'value'
].forEach(name => booleanishAttributeNames.add(name));
...
export const standardizeAttributeName = (name: string): string => {
	let standardAttributeName = standardAttributeNames.get(name);
...
export const createAttribute = <T extends keyof AllAttributes>(rawPropName: T, rawPropValue: AllAttributes[T])
...
export const createClassNameAttribute = (classNames: ClassName | ClassName[]): string | undefined => {
	let classNameValue = '';
...
export const standardizeCssValue = <T extends keyof CSSProperties>(name: T, value: CSSProperties[T]): string => {
...
export const createStyleAttribute = (cssObject: CSSProperties): string | undefined => {
	let styleValue = '';
	...
	for (let i = 0; i < cssNames.length; i++) {
		...
		styleValue += `${hypenateCssName(cssName)}:${standardizeCssValue(cssName, cssValue)};`;
	}
